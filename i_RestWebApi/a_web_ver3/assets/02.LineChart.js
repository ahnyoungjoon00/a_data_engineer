...
export default class LineChart {
    ...
    transformData(datas) {
        ...
        const data = datas.map(e => {
            const date = e.date.substring(5, 11) ; //년-월-일만 추출(x축 tick 라벨로 사용)
        ...
        // mode가 3인 경우, 2023/2024데이터가 반환되어, 연도별 groupby해줘야 비교가 가능해짐
        const grouped = Map.groupBy(data, ({year})=>year); // 연도가 1개인 경우에는 group이 1개 생김
        console.log(grouped); // map객체
        ...
        // 그룹데이터를 array로 변환했고, 두개 이상 연도가 있을 수 있으므로 연도별로 작업해야함
        ...
        const lowTmp = {};
        const cloudData = {};
        data.forEach((e, key) => { //api data 각각 접근해서 data 분리
            date[key] = e.date;
            year[key] = e.year;
            avgTmp[key]  = e.avgTmp;
    ...
    drawInfo(){
        ...
        $('#addInfo').append(str3);
        document.querySelector('#chartBox').innerHTML = ''; // 그래프 영역 지우기
        return true;
    };
    ...
    async drawChart() {
        ...
        // document.querySelector('#chartBox').innerHTML = ''; // 그래프 영역 지우기
        this.drawInfo(); // 연/월 선택할 수 있는 select box 추가 (현재 클래스 메서드)
        $(document).on("click", "#resBtn", async function(){
            // this.drawInfo2(); // 기온 종류 선택 select box 추가
            parent_this.drawInfo2(); // 기온 종류 선택 select box 추가

            parent_this.mode = $("#year").val(); // 선택된 연도에 해당되는 val 반환
            let month = $("#month").val(); // 선택된 월에 해당되는 val 반환
            let url = "";
            ...
            const datas = await fakeApi(); //api data 요청(비동기방식요청 async ~ await)
            console.log(datas);
            [ // parent_this.date, parent_this.tmpData, parent_this.cloudData
                parent_this.date, parent_this.year, 
                parent_this.avgTmp, parent_this.lowTmp, 
                parent_this.highTmp, parent_this.cloudData,
            ] = parent_this.transformData(datas); //현재 클래스 transformData 호출

            drawChartFin(parent_this.date, parent_this.year, parent_this.avgTmp, parent_this.mode); 
            // 그래프 그리기(최초 그래프는) 
    ...
    drawChartFin(date, year, tmpData, mode){
        ...
        var temp1 = { //평균기온 시계열
            ...
            y: Object.values(tmpData[1]), //y축 data는 평균기온
            mode: "lines+markers", //표식이 있는 시계열 그래프
            line: {
                dash:'dot',
                width:2,
        ...
        // =============================================================================
        // layout 구성
        // xticks : x축 값으로 사용할 data 구성, x축은 날짜
        let val = Object.keys(date).map(Number); // 수치형 값 0~59
        let text = Object.values(date);
        console.log(val);
        console.log(text);
